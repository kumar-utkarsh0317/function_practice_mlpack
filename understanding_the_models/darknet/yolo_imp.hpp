...
//main defination for the constructor
template<typename OutputLayerType, typename InitializationRuleType>
YOLO<OutputLayerType, InitializationRuleType>::YOLO(
    ...
    const std::string& weights,
    const bool includeTop) :
    //parameters are asigned to the member variable with list initialization method
...
template<typename OutputLayerType, typename InitializationRuleType>
void YOLO<OutputLayerType, InitializationRuleType>::LoadModel(const std::string& filePath)
{
  //special type of load 
  //"yolo" + yoloVersion this is the name with which the weight is saved and this should same while loading
  //yolo => i guess this is the model in which the parameter is to be loaded
  data::Load(filePath, "yolo" + yoloVersion, yolo);
  Log::Info << "Loaded model." << std::endl;
}

template<typename OutputLayerType, typename InitializationRuleType>
void YOLO<OutputLayerType, InitializationRuleType>::SaveModel(const std::string& filePath)
{
